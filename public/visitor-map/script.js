...
function draw(timestamp = 0) {
  ...
  gradient.addColorStop(0.58, "#e8c6b5");
  gradient.addColorStop(1, "#cb8b73");
  context.beginPath();
  context.arc(center, center, radius, 0, Math.PI * 2);
  context.fillStyle = gradient;
  context.fill();
  context.strokeStyle = "rgba(159,66,44,.35)";
  context.lineWidth = 1;
  context.stroke();
  context.save();
  context.beginPath();
  context.arc(center, center, radius, 0, Math.PI * 2);
  context.clip();
  context.strokeStyle = "rgba(159,66,44,.18)";
  ...
  rows.forEach((row) => {
    ...
    context.beginPath();
    context.arc(point.x, point.y, 3 + Math.sqrt(row.visitors / max) * 7, 0, Math.PI * 2);
    context.fillStyle = "#9f422c";
    context.shadowColor = "rgba(159,66,44,.7)";
...
function animate(timestamp) {
  ...
  if (timestamp - lastFrame >= 32) {
    if (previousTimestamp) rotation = (rotation + (timestamp - previousTimestamp) * 0.0018) % 360;
...
function startAnimation() {
  if (!reduceMotion && !document.hidden && !animationFrame)
    animationFrame = window.requestAnimationFrame(animate);
}
...
function validateCentroids(data) {
  if (!data || typeof data !== "object" || Array.isArray(data))
    throw new Error("invalid centroids");
...
function aggregate(selectedRows) {
  ...
  return [...grouped.values()].sort(
    (a, b) => b.visitors - a.visitors || a.country.localeCompare(b.country),
  );
}
...
function render() {
  ...
  dayPicker.max = latest;
  const selectedRows =
    activeRange === "all" ? sourceRows : sourceRows.filter((row) => row.day === dayPicker.value);
  rows = aggregate(selectedRows);
  const total = rows.reduce((sum, row) => sum + row.visitors, 0);
  totalVisitorsNode.textContent = total.toLocaleString();
  totalCountriesNode.textContent = rows.length.toLocaleString();
  countryCountNode.textContent = `${rows.length} countries`;
  totalLabelNode.textContent =
    activeRange === "all" ? "Total recorded visitors" : "Visitors that day";
  periodLabelNode.textContent =
    activeRange === "all"
      ? days.length
        ? `${readableDay(days[0])} — ${readableDay(latest)}`
        : "Across all available days"
      : readableDay(dayPicker.value);
  globePeriodNode.textContent =
    activeRange === "all" ? "All-time totals" : readableDay(dayPicker.value);
  ...
  rows.forEach((row, index) => {
    ...
    button.type = "button";
    button.className = "country-row";
    button.setAttribute(
      "aria-label",
      `${row.country}, ${row.visitors.toLocaleString()} recorded visitors, open daily history`,
    );
    button.addEventListener("click", () => openCountry(row.countryCode));
    ...
    meter.min = 0;
    meter.max = maximum;
    meter.value = row.visitors;
    meter.setAttribute("aria-hidden", "true");
    button.append(rank, country, visitors, meter);
    item.append(button);
    countriesNode.append(item);
  });
  statusNode.hidden = rows.length > 0;
  statusNode.textContent = rows.length ? "" : "No visitor data is available for this period.";
  draw();
}
...
rangeButtons.forEach((button) =>
  button.addEventListener("click", () => {
    ...
    dateControl.hidden = activeRange !== "day";
    render();
  }),
);
...
async function load() {
  try {
    ...
    const [countryCentroids, data] = await Promise.all([
